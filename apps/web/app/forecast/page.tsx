...
export default function ForecastPage() {
  ...
  useEffect(() => { fetch(`${apiBase}/api/v1/forecast`, { headers: apiHeaders("ar_manager"), credentials: "include" }).then(async response => { if (!response.ok) throw new Error(); setData(await response.json()); }).catch(() => setFailed(true)); }, []);
